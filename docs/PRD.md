...
Responsive Landing Pages from HTML Import or Drag and Drop Builder, Existing Form Mapping plus Native Forms, Multi-Channel Publishing, Script Allowlists, Admin Domains Workflow, and Google OAuth Provisioning
...
- Teams need multiple publishing paths depending on how their website is managed, including WordPress and Drupal subdirectory publishing, and a demo publishing mode for testing.

Replica Pages solves this by combining import and editing, existing-form mapping plus native forms, a normalized submission pipeline, and a publishing system that supports the common real-world flows and edge cases.
...
- Publishing support must include the major flow families:
  ...
  - Custom 404, URL redirects, scheduling, security headers and embedding controls
...
- Domain and DNS friction must be handled with guided setup, clear status, and the ability to publish to a demo domain when production DNS is not ready.
...
- Demo domain publishing for testing
- Custom domain publishing via CNAME to a subdomain
- Subdirectories under a connected subdomain
- WordPress publishing under site subdirectory, via plugin style integration
- Drupal publishing under site subdirectory, via module style integration
...
- Scheduling publish, unpublish, scheduled updates
- SSL and HTTPS management, including CAA edge case handling
- Cloudflare CNAME handling, DNS-only requirement for some setups
- Naked domain and www handling via redirect strategy
- URL redirects capability
- Custom 404 page support per publish target
- Security headers and embedding controls, including X-Frame-Options behavior
...
- Intended for QA and internal review, not long-term production.

### 8.1.2 Custom domain publishing via CNAME
- Support connecting subdomains via CNAME and publishing pages there.
- Include subdirectory publishing under that connected subdomain so a single connected subdomain can host many pages.

### 8.1.3 Cloudflare CNAME compatibility
- If customer uses Cloudflare, support the DNS-only requirement, and surface a clear warning when proxying is enabled.

### 8.1.4 Naked domain and www handling
- Prefer publishing to a subdomain, and support root domain via redirect strategy.
- Support publishing to www when the customer chooses to repoint it and there is no conflicting record.

### 8.1.5 WordPress publishing
- Support a publishing mode where landing pages are served as a subdirectory of the main site, via a WordPress plugin style integration.
- Include troubleshooting surface for common issues like caching and token misconfiguration.

### 8.1.6 Drupal publishing
- Support a publishing mode where landing pages are served as a subdirectory of the main site, via a Drupal module style integration.
...
- Per page scheduling:
  - publish at
  - unpublish at
  - schedule updates to go live at a specific time
...
- If customer has a CAA record, provide required guidance and support to add pki.goog when needed for certificate issuance.
- For WordPress and Drupal publishing, SSL is owned by the CMS side.
...
- Support custom 404 configuration per domain or per publish target:
  - Choose a published page as the 404 page for that domain or site.
- WordPress and Drupal modes:
  - Support mapping a published page as 404 via plugin/module configuration where possible.
...
- Provide an admin setting per domain:
  - Allow embedding (for iframe use cases) or deny embedding (clickjacking protection).
...
- Surface conflict detection and guidance:
  ...
  - Cloudflare proxied versus DNS-only warning
...
### Publishing targets
- Demo domain works for preview and QA.
- Custom domain CNAME works, subdirectory paths work.
- Cloudflare DNS-only guidance surfaced when proxied.
- Naked domain redirect strategy supported.
- Scheduling publishes and unpublishes execute correctly.
- Custom 404 configuration works.
- Redirect manager works for old to new paths.
- Embedding headers can be toggled to allow or deny iframe usage.
