 ...
 * Includes responsive preview, selection highlights, and width scrubber.
 */
...
/* ── Canvas mode: absolute positioning (preview) ── */
      ...
      {childIds.map((id) => {
        ...
        return (
          <div
            ...
            style={{
              position: 'absolute',
              left: (props.x as number) ?? 0,
              top: (props.y as number) ?? 0,
              width: (props.width as number) ?? 200,
              minHeight: (props.height as number) ?? 80,
            }}
          >
...
/* ── Canvas mode: absolute positioning (edit) ── */
        ...
        return (
          <DraggableBlock key={id} blockId={id}>
            ...
            >
              ...
              {isSelected && (
                <div className="absolute -top-5 left-0 text-[10px] font-medium text-primary bg-primary/10 px-1.5 py-0.5 rounded-t">
                  {block.type}
                </div>
              )}
            </div>
...
/* ── Fluid mode: flow layout (edit) with inter-block drop zones ── */

function FluidBlockItem({
  blockId,
  activeId,
  selectedBlockIds,
  handleBlockClick,
  ...
  return (
    <DraggableBlock blockId={blockId}>
      <div
        className={cn(
          'group/block relative transition-all rounded',
      ...
      >
        ...
        {isSelected && block && (
          <div className="absolute -top-5 left-1 text-[10px] font-medium text-primary bg-primary/10 px-1.5 py-0.5 rounded-t z-10">
            {block.type}
          </div>
        )}
      </div>
...
export function EditorCanvas() {
  ...
  const handleDragEnd = (e: DragEndEvent) => {
    ...
    if (overId.startsWith('drop-')) {
      ...
      if (children.indexOf(blockId) >= 0) return;
    ...
    return (
      <div className="flex-1 flex flex-col overflow-hidden bg-muted/30">
        ...
        <div className="flex-1 overflow-auto flex justify-center p-4">
          <div
            className="bg-background shadow-sm border rounded-lg overflow-hidden transition-[width] duration-200"
            style={{ width: canvasWidth, maxWidth: '100%', ...pageStyle }}
          >
  ...
  return (
    ...
    >
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        >
          <div
            className="mx-auto min-h-[400px] bg-background rounded-lg shadow-sm border transition-[width] duration-200"
            style={{ maxWidth: '100%', width: canvasWidth, ...pageStyle }}
          >
            {rootBlock ? (
              isCanvas ? (
                <CanvasEdit
